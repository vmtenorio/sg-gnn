"""
Training time and peak GPU memory on Roman-Empire (22,662 nodes) -- Table XIII
in the paper. Complements the accuracy comparison with a direct time/memory
cost measurement, so scalability is not judged on accuracy alone.

Mirrors ablations/timing_memory.py's methodology (5 warmup epochs, 20 timed
epochs, repeated n_sims=3) but on the large graph and including all 7 models
benchmarked in exp_largegraph_matched.py (GCN, GAT, GRAND, CD-GNN, LG-GNN,
AdaptiveAggGCN-GCN, AdaptiveAggGCN-FBGNN), all at the same hidden dimension
(32) so the cost of SG-GNN's R parallel branches is isolated from any
parameter-fairness adjustment (see exp_largegraph_timing_matched.py for the
complementary measurement at parameter-fair capacity).

Output:
  results/largegraph/<date>-largegraph_timing.pkl
  results/largegraph/<date>-largegraph_timing.json
"""
...
# ── hyperparams (match exp_largegraph.py) ────────────────────────────────────
n_layers = 2
hid_dim  = 32
dropout  = 0.5
lr       = 5e-3
...
# ── load Roman-Empire + build graphs (same as exp_largegraph.py) ────────────
...
model_specs = [
    {'name': 'GCN', 'build': lambda: GCN(in_dim, hid_dim, num_classes, n_layers,
                                          nonlin=nonlin, last_act=nn.Identity(),
                                          dropout=dropout, gcnlayer=GCNConv).to(device),
     'X': X_hom, 'ei': ei_orig, 'fw_kwargs': {}},
    {'name': 'GAT', 'build': lambda: GCN(in_dim, hid_dim, num_classes, n_layers,
                                          nonlin=nonlin, last_act=nn.Identity(),
                                          dropout=dropout, gcnlayer=GATConv).to(device),
     'X': X_hom, 'ei': ei_orig, 'fw_kwargs': {}},
    {'name': 'GRAND', 'build': lambda: GRANDWrapper(in_dim, hid_dim, num_classes, n_layers,
                                                     dropout, nonlin, last_act_softmax).to(device),
     'X': X_hom, 'ei': ei_orig, 'fw_kwargs': {}},
    {'name': 'CD-GNN', 'build': lambda: CDGNNWrapper(in_dim, hid_dim, num_classes, n_layers,
                                                      dropout, nonlin, last_act_softmax).to(device),
     'X': X_hom, 'ei': ei_orig, 'fw_kwargs': {}},
    {'name': 'LG-GNN', 'build': lambda: LGGNNWrapper(in_dim, hid_dim, num_classes, n_layers,
                                                      dropout, nonlin, last_act_softmax).to(device),
     'X': X_hom, 'ei': ei_orig, 'fw_kwargs': {}},
    {'name': 'AdaptiveAggGCN-GCN', 'build': lambda: AdaptiveAggGCN(
    ...
    {'name': 'AdaptiveAggGCN-FBGNN', 'build': lambda: AdaptiveAggGCN(
...
fname = str(OUT_DIR / f'{datestamp}-largegraph_timing')
